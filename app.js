...
document.querySelector('#rebalance-all').addEventListener('click', async () => {
  ...
  try {
    button.disabled = true;
    button.textContent = 'Requesting rebalance…';
    const result = await post('/glider/rebalance', { portfolioId });
    button.textContent = 'Rebalance requested ✓';
    alert(`Glider accepted the rebalance request. Operation: ${result.data.operationId || 'processing'}`);
  } catch (error) {
    button.disabled = false;
    button.textContent = 'Rebalance investment';
    alert(`Rebalance was not completed: ${error.message || 'Unknown error'}`);
  }
});
